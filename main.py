...
class Item(db.Model):
    ...
    image_url = db.Column(db.Text, nullable=True)  # Изменено на Text
    ...
    if request.method == 'POST':
        ...
        for image in images:
            if image and image.filename != '':
                filename = secure_filename(image.filename)
                image.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                image_urls.append(url_for('static', filename=f'img/{filename}'))

        image_urls = "/$-$/".join(image_urls)  # Объединяем URL-адреса изображений

        new_item = Item(name=name, price=price, description=description, category=category, subcategory=subcategory,
...
def item_detail(item_id):
    item = Item.query.get_or_404(item_id)
    print(item.image_url)  # Отладочная печать
